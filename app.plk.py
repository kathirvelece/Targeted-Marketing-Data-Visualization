...
# ✅ Load model correctly from tuple
with open('model.pkl', 'rb') as f:
    model_tuple = pickle.load(f)
    model = model_tuple[0]  # extract model from (model, feature_names)
...
def predict():
    try:
        ...
        # ✅ Form input array (must match training order)
        input_data = np.array([[age, income, spending_score, membership_years, purchase_freq, last_purchase_amount]])

        # ✅ Make prediction and force positive result
        prediction = model.predict(input_data)[0]
        prediction = round(abs(prediction), 2)  # force positive CLV
